...
datasets_kwargs={
    'GrabCut' :{'dataset_path':'Data/GrabCut' ,'img_folder':'data_GT','gt_folder':'boundary_GT','threshold':128,'ignore_label':128 },
    'Berkeley':{'dataset_path':'Data/Berkeley','img_folder':'images' ,'gt_folder':'masks'      ,'threshold':128,'ignore_label':None},
    'DAVIS'   :{'dataset_path':'Data/DAVIS'   ,'img_folder':'img'    ,'gt_folder':'gt'         ,'threshold':0.5,'ignore_label':None},
    'VOCdevkit' :{'dataset_path':'Data/VOCdevkit/VOC2012'},
}
default_miou_targets={'GrabCut':0.90,'Berkeley':0.90,'DAVIS':0.90,'VOCdevkit':0.85}
...
def eval_dataset(model, dataset, max_point_num=20, record_point_num=20,if_sis=False,miou_target=None,if_cuda=True):
    ...
    if dataset in datasets_kwargs:
        dataset_iter= VOC2012(**datasets_kwargs[dataset]) if dataset=='VOCdevkit' else  Dataset(**datasets_kwargs[dataset]) 
        miou_target = default_miou_targets[dataset] if miou_target is None else miou_target
    else:
        dataset_iter=Dataset(dataset_path='Data/{}'.format(dataset)) 
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', type=str, default='VOCdevkit', help='evaluation dataset (default: VOCdevkit)')
    ...
    if Path('Data/{}'.format(args.dataset)).exists():
        model = init_model('fcanet', args.backbone, '/home/aistudio/fcanet/PaddlePretrainedModel/fcanet-{}.pdparams'.format(args.backbone),if_cuda=not args.cpu)
        eval_dataset(model, args.dataset, if_sis=args.sis, miou_target=(None if args.miou<0 else args.miou), if_cuda=not args.cpu)
    else:
        print('not found folder [Data/{}]'.format(args.dataset))
