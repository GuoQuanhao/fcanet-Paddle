...
class ToTensor(object):
    ...
    def __call__(self, sample):
        for elem in sample.keys():
            ...
            tmp = paddle.to_tensor(tmp, dtype='float32')
            tmp = tmp / 255.0 if self.if_div else tmp
            sample[elem] = tmp                          
        return sample
...
def init_model(model_name='fcanet',backbone='resnet',pretrained_file=None, if_cuda=True):
    ...
    if pretrained_file is not None:
        if if_cuda:
            ...
            state_dict=paddle.load(pretrained_file,map_location=lambda storage, loc: storage)
...
def predict(model, img, seq_points,  if_sis=False, if_cuda=True):
    ...
    with paddle.no_grad():
        output = model(input)
    result = paddle.nn.functional.sigmoid(output).numpy()[0,0,:,:]
