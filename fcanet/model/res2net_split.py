...
class Bottle2neck(nn.Layer):
    ...
    def __init__(self, inplanes, planes, stride=1, dilation=1,
        ...
        for i in range(self.nums):
          convs.append(nn.Conv2D(width, width, kernel_size=3, stride=stride, dilation=dilation, padding=dilation, bias_attr=False))
    ...
    def forward(self, x):
        ...
        out = self.bn1(out)
        out = self.relu(out)
        spx = paddle.split(out, out.shape[1] // self.width, 1)
        for i in range(self.nums):
...
class Res2NetSplit(nn.Layer):
    ...
    def __init__(self, block, layers, output_stride, BatchNorm, layer_range=[0,4], aux_channel=0, baseWidth = 26, scale = 4):
        ...
        if layer_range[0]<=0 and 0<=layer_range[1]:
            self.conv1 = nn.Sequential(
                nn.Conv2D(self.inplanes, 32, 3, 2, 1, bias_attr=False),
                BatchNorm(32),
                nn.ReLU(),
                nn.Conv2D(32, 32, 3, 1, 1, bias_attr=False),
                BatchNorm(32),
                nn.ReLU(),
    ...
    def _load_pretrained_model(self, pretrain_path):
...
def Res2NetSplit50(output_stride, BatchNorm, pretrained=False,layer_range=[0,4], aux_channel=0):
    ...
    if pretrained:
        model._load_pretrained_model('home/aistudio/InitialPaddleModel/res2net50_v1b_26w_4s.pdparams')
    return model
...
def Res2NetSplit101(output_stride, BatchNorm, pretrained=False,layer_range=[0,4], aux_channel=0):
    ...
    if pretrained:
        model._load_pretrained_model('home/aistudio/InitialPaddleModel/res2net101_v1b_26w_4s.pdparams')
    return model
